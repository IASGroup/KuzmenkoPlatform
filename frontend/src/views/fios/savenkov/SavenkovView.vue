<template>
  <v-card>
    ...
    <v-card class="ml-5 mr-5 mb-5 px-5">
        <h2 class="text-center">Процентное содержание статей в категориии по триграммам</h2>
        <v-autocomplete
            class="w-100"
            clearable
        ...
        <v-pagination
            ...
            @input="onPageChange"
        />
        <div id="percentageTrigrams"/>
    </v-card>
    <v-spacer></v-spacer>
    <v-card class="h-50 w-100 px-5 my-10 mx-5">
      <h2 class="text-center">Кластеризация по уникальности триграмм</h2>
      <v-row class="w-100">
        <v-col class="w-50">
    ...
    <v-spacer></v-spacer>
    <v-card class="h-50 w-100 px-5 my-10 mx-5">
      <h2 class="text-center">Кластеризация по уникальности триграмм с использованием методов по уменьшению размерности признаков объектов</h2>
      <v-row class="w-100">
        <v-col class="w-50">
    ...
    <v-spacer></v-spacer>
    <v-card class="mx-5">
      <h2 class="text-center">Кластеризация по количеству биграмм и триграмм в категории</h2>
      <div id="сlustering_count_n_grams_in_category"/>
    </v-card>
  </v-card>
</template>
...
import {
  ...
  getClusteringPca,
  getClusteringTsne,
  getClusteringCountNGramsInCategory,
  getCategories,
  getPercentage,
} from '@/api/savenkov';
...
onMounted(async () => {
  ...
  getClusteringCountNGramsInCategory().then(value => Plotly.newPlot(document.getElementById("сlustering_count_n_grams_in_category"),JSON.parse(value)))
...
watch(selectedIdCategory, async()=>{
  ...
  Plotly.newPlot(document.getElementById("percentageTrigrams"), [{
